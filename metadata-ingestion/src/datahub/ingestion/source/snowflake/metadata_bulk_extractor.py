...
class BulkMetadataExtractor:
    ...
    def __init__(
        ...
        self._temp_dir = tempfile.mkdtemp(prefix="snowflake_metadata_")
        logger.info(
    ...
    def extract_database(self, database_name: str) -> pd.DataFrame:
        ...
        """
        logger.info(f"📦 Extracting metadata for database: {database_name}")

        try:
            # Execute database-level extraction (optimized SQL)
            logger.info("📍 Step 1: Executing database-level extraction...")
    ...
    def extract_all_metadata(
        ...
        if databases is None:
            ...
            logger.info(f"Will extract metadata for {len(databases)} databases")
        else:
            logger.info(f"Filtering to {len(databases)} specified databases")
        ...
        logger.info("⏭️ Schema discovery via SQL (no SHOW commands)")
    ...
    def _extract_all_metadata_account_level(
        self,
        databases: List[str],
        schemas: List[str],
    ) -> pd.DataFrame:
        """Extract all metadata in a single account-level query (small accounts)."""
        stage_name = None

        try:
            # Create stage and file format (use first database for context)
            stage_name = self._create_stage(
                database=databases[0] if databases else None
            )
            file_format = "PARQUET"

            # Execute account-level extraction
            logger.info("📍 Step 1: Executing account-level metadata extraction...")
            output_path = self._execute_metadata_extraction(
                stage_name, file_format, databases, schemas
            )

            # Download file
            logger.info("📍 Step 2: Downloading metadata file...")
            local_file = self._download_file(output_path)

            # Parse and return
            logger.info("📍 Step 3: Parsing metadata...")
            df = self._parse_metadata_file(local_file)

            logger.info(
                f"✅ Account-level extraction complete: {len(df)} metadata rows"
            )
            return df
    ...
    def _extract_all_metadata_database_level_sequential(
        ...
        for i, db in enumerate(databases, 1):
            try:
                logger.info(f"[{i}/{len(databases)}] Extracting {db}...")
    ...
    def _extract_all_metadata_database_level_parallel(
        self,
        databases: List[str],
    ) -> pd.DataFrame:
        """Parallel extraction using ThreadPoolExecutor (with connection pool)."""
        logger.info(
            f"Using parallel extraction with {self._max_workers} worker threads"
        )
        dfs = []
        start_time = time.time()

        def _extract_one(db: str) -> Optional[pd.DataFrame]:
            """Extract metadata for one database using a dedicated connection."""
            try:
                # Each thread gets its own connection
                conn = self._connection_config.get_connection()
                try:
                    extractor = BulkMetadataExtractor(
                        conn, connection_config=None, max_workers=1
                    )
                    extractor._temp_dir = self._temp_dir  # Share temp directory
                    return extractor.extract_database(db)
                finally:
                    conn.close()
            except Exception as e:
                logger.warning(f"Failed to extract {db}: {e}")
                return None
            ...
            for i, future in enumerate(as_completed(futures), 1):
                ...
                try:
                    ...
                    if df is not None and not df.empty:
                        dfs.append(df)
                        logger.info(f"[{i}/{len(databases)}] ✅ {db}")
                    else:
                        logger.warning(f"[{i}/{len(databases)}] ⚠️ {db} returned empty")
                except Exception as e:
                    logger.warning(f"[{i}/{len(databases)}] ⚠️ {db} failed: {e}")
        ...
        # Enrich with constraints (2 queries for all databases)
        ...
        logger.info(
            f"✅ Database-level extraction complete: {len(combined_df)} metadata rows "
            f"from {len(dfs)}/{len(databases)} databases "
            f"(⏱️ {elapsed:.1f}s)"
        )
    ...
    def _extract_all_metadata_database_level_batched(
        ...
        def signal_handler(signum: int, frame) -> None:
            logger.warning(
                "\n⛔ Received interrupt signal (Ctrl+C). Stopping extraction..."
            )
            with result_lock:
        ...
        try:
            ...
            logger.debug("Signal handler restored")
    ...
    def _execute_metadata_extraction(
        self,
        stage_name: str,
        file_format: str,
        databases: Optional[List[str]] = None,
        ...
        """
        Execute the account-level metadata extraction query and export to stage.

        Returns path to the exported file.
        """
        # Load the SQL script
        script_path = Path(__file__).parent / "metadata_extractor.sql"
        if not script_path.exists():
            raise FileNotFoundError(f"Metadata extractor SQL not found: {script_path}")
        ...
        if databases or schemas:
            ...
            if databases:
                ...
                where_parts.append(f"database_name IN ({db_list})")
            if schemas:
                schema_list = ", ".join(f"'{s}'" for s in schemas)
                where_parts.append(f"schema_name IN ({schema_list})")
            where_clause = " AND ".join(where_parts)
            extraction_query = f"{extraction_query} WHERE {where_clause}"

        # Create file format for output
        self.connection.query(
            f"""
            CREATE OR REPLACE FILE FORMAT datahub_metadata_format
            TYPE = '{file_format}'
            COMPRESSION = 'SNAPPY'
            """
        )

        # Execute extraction and output to stage
        timestamp = pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")
        output_filename = f"metadata_{timestamp}.{file_format.lower()}"

        copy_command = f"""
        COPY INTO @{stage_name}/{output_filename}
        FROM ({extraction_query})
        FILE_FORMAT = datahub_metadata_format
        OVERWRITE = TRUE
        """

        logger.debug(f"Executing COPY INTO: {copy_command[:100]}...")
        cursor = self.connection.query(copy_command)
        result = cursor.fetchall()
        logger.debug(f"COPY result: {result}")

        return f"@{stage_name}/{output_filename}"

    def _download_file(self, stage_path: str) -> str:
        """
        Download file from Snowflake stage to local temp directory.

        Note: stage_path is already a local file path when using direct query streaming.
        This method is kept for compatibility but is effectively a no-op.
        """
        # When using direct query streaming, stage_path is already a local file
        return stage_path

...
| [Domains](../../../domains.md) | ❌ | Not supported. Domains can be set manually in DataHub after ingestion. |
...
Optional **warehouse (physical table) lineage** for Intelligent Cubes: set `include_warehouse_lineage: true`. The warehouse platform is detected automatically by querying `/api/datasources` — no manual platform configuration is needed. Optionally set `warehouse_lineage_database` and `warehouse_lineage_schema` to qualify bare table names so dataset URNs match your warehouse ingestion. The connector parses table names from `GET /api/v2/cubes/{id}/sqlView` SQL output and emits `upstreamLineage` on the cube dataset.
...
| <div className="path-line"><span className="path-main">cube_pattern</span></div> <div className="type-name-line"><span className="type-name">AllowDenyPattern</span></div> | Regex patterns to filter Intelligent Cubes by name after listing. Reduces per-cube API calls (schema, warehouse lineage) but not the cube search itself. <div className="default-line default-line-with-docs">Default: <span className="default-value">Allow all</span></div> |
| <div className="path-line"><span className="path-main">include_folders</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | When false, do not call the folders API or emit folder containers. <div className="default-line default-line-with-docs">Default: <span className="default-value">True</span></div> |
| <div className="path-line"><span className="path-main">include_dashboards</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | When false, skip dashboard (dossier) search and processing. Largest API call saver for environments with many dashboards. <div className="default-line default-line-with-docs">Default: <span className="default-value">True</span></div> |
| <div className="path-line"><span className="path-main">include_reports</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | When false, skip report search and chart emission. <div className="default-line default-line-with-docs">Default: <span className="default-value">True</span></div> |
| <div className="path-line"><span className="path-main">include_cubes</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | When false, skip emitting cube datasets. Cube search still runs when `include_lineage` and `include_reports` are true (needed for report→cube lineage resolution). <div className="default-line default-line-with-docs">Default: <span className="default-value">True</span></div> |
| <div className="path-line"><span className="path-main">include_datasets</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | When false, skip the project datasets API call and library dataset emission. Dataset registry is still populated when `include_lineage` and `include_reports` are true. <div className="default-line default-line-with-docs">Default: <span className="default-value">True</span></div> |
...
| <div className="path-line"><span className="path-main">include_cube_view_sql</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | When true, call the cube sqlView API and emit a SQL view definition for each Intelligent Cube. Required for warehouse lineage and column-level lineage. <div className="default-line default-line-with-docs">Default: <span className="default-value">True</span></div> |
| <div className="path-line"><span className="path-main">include_field_formulas</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | When true, fetch attribute and metric expressions from the modeling API and surface them as field descriptions. Attribute expressions show the physical column name and backing tables; metric expressions show the aggregation formula (e.g. `Sum(NET_SLS_RTL_AMT)`). Adds one API call per unique attribute/metric. <div className="default-line default-line-with-docs">Default: <span className="default-value">True</span></div> |
| <div className="path-line"><span className="path-main">include_report_definitions</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | When true, call the full report definition API for each matched report. Enables report→cube lineage, report schema (attributes/metrics), and enriched custom properties. Disabled by default because it adds one extra API call per report — expensive at scale. Enable only for scoped ingestion runs (with `report_pattern` filters) or when cube→report lineage is required. <div className="default-line default-line-with-docs">Default: <span className="default-value">False</span></div> |
| <div className="path-line"><span className="path-main">include_column_lineage</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | Extract column-level lineage from cube SQL. Requires `include_cube_schema`, `include_lineage`, and `include_warehouse_lineage` to all be true. The warehouse platform is detected automatically. <div className="default-line default-line-with-docs">Default: <span className="default-value">True</span></div> |
| <div className="path-line"><span className="path-main">include_warehouse_lineage</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | When true, emit upstream lineage from physical warehouse tables to cubes, reports, and documents. The source platform is detected automatically from `/api/datasources`; if that endpoint returns 403 it is inferred from SQL quoting style. No manual platform configuration is required. <div className="default-line default-line-with-docs">Default: <span className="default-value">False</span></div> |
| <div className="path-line"><span className="path-main">warehouse_lineage_database</span></div> <div className="type-name-line"><span className="type-name">string</span></div> | Default database/catalog name for building dataset URNs when the modeling API does not provide a full namespace. <div className="default-line default-line-with-docs">Default: <span className="default-value">None</span></div> |
| <div className="path-line"><span className="path-main">warehouse_lineage_schema</span></div> <div className="type-name-line"><span className="type-name">string</span></div> | Default schema name for building dataset URNs when the modeling API only returns a table name. <div className="default-line default-line-with-docs">Default: <span className="default-value">None</span></div> |
| <div className="path-line"><span className="path-main">include_unloaded_projects</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | When false (default), only projects with `status: 0` (loaded on IServer) are ingested. Set to true to include idle/unloaded projects, which may trigger IServer ERR001-style errors. <div className="default-line default-line-with-docs">Default: <span className="default-value">False</span></div> |
| <div className="path-line"><span className="path-main">cube_search_object_type</span></div> <div className="type-name-line"><span className="type-name">integer</span></div> | Object type code passed to `/api/searches/results` when listing cubes. `776` is the standard MicroStrategy Library value for cube-style objects; override if your server expects a different type. <div className="default-line default-line-with-docs">Default: <span className="default-value">776</span></div> |
| <div className="path-line"><span className="path-main">preflight_dashboard_exists</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | Before fetching each dashboard definition, call the objects API to verify the dashboard still exists. Skips the dashboard on 404. Increases API traffic; useful when troubleshooting missing or recently-deleted objects. <div className="default-line default-line-with-docs">Default: <span className="default-value">False</span></div> |
| <div className="path-line"><span className="path-main">max_workers</span></div> <div className="type-name-line"><span className="type-name">integer</span></div> | Maximum number of threads used to pre-fetch cube metadata (sqlView + schema) in parallel. Set to `1` to disable parallelism for easier debugging. Values above 10 risk hitting MicroStrategy REST API rate limits. <div className="default-line default-line-with-docs">Default: <span className="default-value">4</span></div> |
...
Column-level lineage from warehouse tables to cubes is supported when `include_column_lineage: true`, `include_warehouse_lineage: true`, and `include_cube_view_sql: true` are all set. The connector uses SQL parsing on the cube's SQL view to extract column-level upstream edges.

Column-level lineage from cubes to dashboard visualizations (cube field → chart column) is not currently supported.

- **Affects**: Fine-grained column lineage from cube to chart
